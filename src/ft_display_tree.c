...
/*   Updated: 2017/06/03 06:39:51 by sbonnefo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_display_tree(t_direct *node, t_direct *prev, t_direct *dad,
									t_opt *op)
{
	if (!node)
		return ;
	if (node->left)
		ft_display_tree(node->left, prev, dad, op);
	ft_display_indir_node(node, dad, op);
	if (node->right)
		ft_display_tree(node->right, prev, dad, op);
}
